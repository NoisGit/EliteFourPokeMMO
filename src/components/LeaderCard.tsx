...
export const LeaderCard = ({ leader, isExpanded, onClick }: LeaderCardProps) => {
  ...
  return (
    <button
      ref={cardRef}
      type="button"
      className={`gsap-leader-card group relative min-w-0 overflow-hidden rounded-xl border bg-slate-950/75 p-2 text-left shadow-lg shadow-black/20 transition-colors duration-300 will-change-transform hover:border-rose-200/50 hover:bg-slate-900/95 active:scale-[0.98] sm:rounded-3xl sm:p-3 ${
        isExpanded
